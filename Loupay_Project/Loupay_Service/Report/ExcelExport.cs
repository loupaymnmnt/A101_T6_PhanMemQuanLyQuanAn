    ...
    public class ExcelExport
    {
        ...
        private void BuildReplacerCurrentDate(ref Dictionary<string, string> pReplacer)
        {
            if (pReplacer != null)
            {
                ...
                pReplacer.Add("%NgayThangNam2", ngay);
                pReplacer.Add("%NgayThangNam3", ngay);
                //pReplacer.Add("%TongSo", "600");
            }
        }
        ...
        public bool ExportHoaDon(List<CurrentOrder> orders, ref string fileName, bool isPrintPreview)
        {
            //Check if data is null
            if (orders == null || (orders != null && orders.Count == 0))
            {
                return false;
            }
            ...
            decimal sum = 0;
            foreach (CurrentOrder order in orders)
            {
                sum += order.TongGia;
            }
            replacer.Add("%TongSo", sum.ToString());
            replacer.Add("%TenNguoiLap", CurrentAccount.Instance.TenNguoiDung);
            //    replacer.Add(V_TONGSO, "100");
            BuildReplacerCurrentDate(ref replacer);

            return OutSimpleReport(orders, replacer, "Mon", isPrintPreview, ref fileName);
        }
        #endregion
    }
}
